...
- **Merge Sort**: A highly efficient and stable divide-and-conquer algorithm. (**O(n log n)**)
- **Quick Sort (Iterative)**: A very fast divide-and-conquer algorithm, implemented iteratively to handle large datasets without recursion limits. (**O(n log n)** on average)
- **Heap Sort**: An efficient comparison-based sort that uses a binary heap data structure. (**O(n log n)**)
...
- **The O(n²) Struggle**: Insertion Sort starts fast on small arrays but its performance degrades exponentially as the input size grows, making it impractical for large datasets.
- **The Power of O(n log n)**: Merge Sort, Heap Sort, and Quick Sort are significantly faster and scale much more effectively, proving why they are staples in computer science.
